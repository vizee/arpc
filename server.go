...
func (b *ServerBuilder) Register(svc interface{}) *ServerBuilder {
	...
	if b.services[svcName] != nil {
		b.err = fmt.Errorf("service %s already defined", svcName)
		return b
	}

	methods := collectRpcMethods(svcType)
	if len(methods) == 0 {
		b.err = fmt.Errorf("type %s has no suitable RPC method", svcType.Name())
		return b
	}
	...
	b.services[svcName] = &serviceDesc{
		rcvr:    svcVal,
		methods: methods,
	}
	return b
